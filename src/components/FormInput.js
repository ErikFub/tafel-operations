export default function FormInput({ label, id, placeholder, required, type, autofocus, defaultValue }) {
    ...
    return (
        <div>
            <label for={idAdj} className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{label}{required && <span className="text-red-600 font-medium text-sm"> *</span>}</label>
